...
public class Pageshownum implements Pageintr{
	...
	public String getpagecontent(int page,int maxn,String s) {
		...
		// TODO Auto-generated method stub
		 HTML=HTML+"<div>";
		   HTML=HTML+"<a class=\"page_button\" href=\"http://localhost:8084/searchengine/searching?username="+s+"&page="+1+
				   "\">首页</a>";
		   if(page>1) {
			   HTML=HTML+"<a class=\"page_button\" href=\"http://localhost:8084/searchengine/searching?username="+s+"&page="+(page-1)+
					   "\">上一页</a>";
		   }
		   else {
		   ...
		   if(page>4) {
			   for(int j=page-3;j<page;j++) {
				   HTML=HTML+"<a class=\"page_button\" href=\"http://localhost:8084/searchengine/searching?username="+s+"&page="+j+
						   "\">"+j+"</a>";
			   }
		   }
		   else if(page>maxn-6) {
		   ...
		   HTML=HTML+"<a class=\"page_button\" href=\"http://localhost:8084/searchengine/searching?username="+s+"&page="+maxn+
				   "\">尾页</a></div>";
		return HTML;
	}
